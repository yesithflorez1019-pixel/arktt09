...
import { 
  Monitor, Calendar, ArrowRight, Shield, Heart, Smile, CheckCircle, MapPin, 
  Facebook, Instagram, FileText, CreditCard, Clock, AlertCircle 
} from 'lucide-react';
...
// COMPONENTES UI
import { Seccion, TituloSeccion, Boton } from '../components/UI';
...
export default function Inicio() {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        const snapNoticias = await getDocs(qNoticias);
        setNoticiasDestacadas(snapNoticias.docs.map(doc => ({ id: doc.id, ...doc.data() })));

        // Cargar Eventos
        const qEventos = query(collection(db, "eventos"), orderBy("fecha", "asc"), limit(3));
        const snapEventos = await getDocs(qEventos);
        setEventosHome(snapEventos.docs.map(doc => ({ id: doc.id, ...doc.data() })));
  ...
  // Función para estado del evento (Comparando fechas)
  const getEstado = (fechaISO) => {
    const f = new Date(fechaISO).setHours(0,0,0,0);
    const h = new Date().setHours(0,0,0,0);
    
    if (f === h) return { txt: '¡HOY!', color: 'bg-green-500 animate-pulse' };
    if (f < h) return { txt: 'FINALIZADO', color: 'bg-slate-400' };
    return { txt: 'PRÓXIMAMENTE', color: 'bg-celeste-500' };
  };
  ...
  return (
    <div className="font-sans text-slate-800 bg-slate-50 overflow-x-hidden">
      ...
      {/* HERO SECTION */}
      <section className="relative h-[550px] md:h-[650px] lg:h-[720px] pb-16 bg-white group">
        <Swiper
      ...
      {/* PRÓXIMOS EVENTOS (NUEVO: Usa datos de Firebase) */}
      <Seccion blanca={true}>
         <div className="flex flex-col md:flex-row justify-between items-end mb-12">
             {/* blanco={false} para que el texto sea AZUL OSCURO */}
             <TituloSeccion 
                titulo="Próximos Eventos" 
                subtitulo="Agéndate con nosotros" 
                blanco={false} 
             />
             <button onClick={() => navigate('/agenda')} className="hidden md:flex bg-celeste-50 hover:bg-celeste-100 text-celeste-700 border border-celeste-200 px-4 py-2 rounded-lg font-bold transition items-center gap-2">
         ...
         <div className="grid md:grid-cols-3 gap-6">
            {eventosHome.map(ev => {
               const estado = getEstado(ev.fecha);
               // Fecha para mostrar la hora
               const fechaObj = new Date(ev.fecha);
               
               return (
                 <div key={ev.id} className="bg-white rounded-2xl overflow-hidden shadow-lg border border-slate-100 flex flex-col h-full group cursor-pointer hover:-translate-y-2 transition-all duration-300" onClick={() => navigate(`/agenda/evento/${ev.id}`)}>
                    <div className="h-48 relative overflow-hidden flex-shrink-0">
                       <img src={ev.imagen} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt="Evento"/>
                       {/* --- CORRECCIÓN: Estado visible arriba a la derecha --- */}
                       <div className={`absolute top-3 right-3 ${estado.color} text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-md`}>
                          {estado.txt}
                       </div>
                    </div>
                    <div className="p-6 flex flex-col flex-grow">
                       {/* Categoría o Lugar */}
                       <span className="text-celeste-600 text-xs font-bold uppercase tracking-wider flex items-center gap-1 mb-2">
                          <MapPin size={12}/> {ev.lugar || 'Evento Institucional'}
                       </span>

                       <h4 className="font-bold text-xl text-slate-800 leading-tight mb-3 group-hover:text-celeste-600 transition-colors">{ev.titulo}</h4>
                       
                       <p className="text-slate-500 text-sm mb-4 line-clamp-2 flex-grow">{ev.resumen}</p>
                       
                       {/* --- CORRECCIÓN: Mostramos fecha y hora --- */}
                       <div className="flex items-center gap-2 text-xs text-celeste-600 font-bold bg-celeste-50 p-2 rounded-lg mt-auto">
                          <Clock size={14}/> 
         ...
         <div className="mt-8 text-center md:hidden">
            <Boton variante="azul" full={true} onClick={() => navigate('/agenda')}>Ver Agenda Completa</Boton>
         </div>
      </Seccion>
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            <div>
              ...
              <p className="text-celeste-100 text-lg mb-8 leading-relaxed">
                En el Liceo Formador de Exploradores, no solo educamos la mente, sino también el corazón.
              </p>
              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  <div className="p-3 bg-white/10 rounded-xl backdrop-blur-sm"><Shield className="text-celeste-100" size={24} /></div>
                  <div><h4 className="text-xl font-bold mb-1">Instalaciones Seguras</h4><p className="text-celeste-100 text-sm">Espacios monitoreados.</p></div>
                </div>
                <div className="flex items-start gap-4">
                  <div className="p-3 bg-white/10 rounded-xl backdrop-blur-sm"><Smile className="text-celeste-100" size={24} /></div>
                  <div><h4 className="text-xl font-bold mb-1">Educación Feliz</h4><p className="text-celeste-100 text-sm">El juego como base.</p></div>
                </div>
                <div className="flex items-start gap-4">
                  <div className="p-3 bg-white/10 rounded-xl backdrop-blur-sm"><Heart className="text-celeste-100" size={24} /></div>
                  <div><h4 className="text-xl font-bold mb-1">Acompañamiento</h4><p className="text-celeste-100 text-sm">Docentes comprometidos.</p></div>
                </div>
              </div>
              <button onClick={() => navigate('/galeria')} className="mt-10 bg-white text-celeste-700 font-bold py-3 px-8 rounded-full shadow-lg hover:bg-celeste-50 transition-colors">
          ...
          <div className="bg-slate-100 rounded-[2.5rem] shadow-2xl overflow-hidden max-w-5xl mx-auto border-4 border-white/20">
            <div className="flex flex-col md:flex-row h-full">
              <div className="md:w-1/2 h-80 md:h-auto relative">
                <img src="/fotos-inicio/7.jpeg" alt="Sede" className="w-full h-full object-cover"/>
              ...
              <div className="md:w-1/2 p-10 md:p-14 flex flex-col justify-center bg-white">
                ...
                <p className="text-slate-600 mb-8 leading-relaxed">Contamos con un espacio unificado moderno y seguro.</p>
